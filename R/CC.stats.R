...
#'
#' Provides the CC size distribution, which is the number of CCs including a given
#'number of protein members, and the proportion of single- vs multi-protein CCs.
...
#' @param reducedIncM a \code{logical} value indicating if the input matrix is the
#' original complete incidence matrix or a reduced version of it which only contains
#' proteins with at least one shared peptide and all peptides mapping on
#' such proteins (generated by the \code{\link{reduceIncM}} function.
#' @return a \code{list} of four outputs: i. a \code{vector} containing all those
#' proteins which belong to single-protein connected component (each protein is
#' a single-protein connected component); ii. an \code{integer} representing the
#' number of single-protein CCs; iii. an \code{integer} representing the number of
#' multi-protein CCs; iv. a \code{data.frame} describing size distribution of
#' connected components (number of connected components per number of protein members)
#' @examples
...
#' CCstatsOut <- CC.stats(incM = incM_reduced
...
CC.stats <- function(incM, cc.proteins, reducedIncM){
  ...
  N_proteins_N_CCs <- as.data.frame(table(N_proteins))
  colnames(N_proteins_N_CCs) <- c("N_proteins", "N_CCs")
  N_proteins_N_CCs$N_proteins <- as.numeric(as.vector(N_proteins_N_CCs$N_proteins))
  ...
  if(isTRUE(reducedIncM)){
    ...
    N_proteins_N_CCs <- rbind(N_proteins_N_CCs, c(1, N_singleProtCC))

  }else{
    N_singleProtCC <- sum(N_proteins_N_CCs[N_proteins_N_CCs$N_proteins==1,]$N_CCs)
    N_multiProtCC <- sum(N_proteins_N_CCs[N_proteins_N_CCs$N_proteins>1,]$N_CCs)
    N_proteins_N_CCs <- rbind(N_proteins_N_CCs, c(1, N_singleProtCC))
  }
  ...
  NproteinsDistribution=merge(proteins, N_proteins_N_CCs, by="N_proteins", all.x = T)
  NproteinsDistribution[is.na(NproteinsDistribution)] <- 0
  NproteinsDistribution <- rbind(NproteinsDistribution, c(">10", sum(N_proteins_N_CCs[N_proteins_N_CCs$N_proteins>10,]$N_CCs)))
  ...
  return(list(singleProtCC = singleProtCC
              ...
              , NproteinsDistribution = NproteinsDistribution))
