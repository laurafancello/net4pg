---
title: "An introduction to CCs4prot"
author: 
  - name: Laura Fancello
...
fig_height: 10
vignette: >
  %\VignetteIndexEntry{An introduction to CCs4prot}
...
knitr::opts_chunk$set(collapse = TRUE, comment = "#>", cache.lazy = TRUE, cache=TRUE)
```
...
other by a path and not connected to any other of the vertices in the supergraph. Proteins sharing one or more peptides are gathered in the same CC (multi-protein CCs), while proteins exclusively identified by specific peptides represent CCs with a single protein vertex (single-protein CCs). The proportion of multi-protein CCs and their size (*i.e.*, number of protein members) can be used as a measure of the ambiguity of protein identifications.   

  Nowadays, an increasing number of sample-matched proteomic and transcriptomic datasets is available, which can be exploited to deal with ambiguity of protein identifications. Indeed, according to the central dogma of biology, there can be no protein without the corresponding transcript, so that protein identifications for which the corresponding transcript is identified in the sample-matched transcriptome are more likely to be correct than protein identifications with no expressed transcript.
  
  The CCs4prot package implements two main functionalities:  
    
  **1. a pipeline to build, from proteomic identifications, a bipartite graph of peptide-to-protein mappings and efficiently calculate graph CCs to assess ambiguity of protein identifications**. CCs represent a peptide-centric strategy to group proteins and it is independent from the variety of protein-centric strategies of protein grouping and protein inference. As such, it does not require protein inference and it is widely applicable, reproducible and transparent.   
  **2. a transcriptome-informed filtering strategy which exploits sample-matched transcriptome information to reduce ambiguity of protein identifications in shotgun proteomics**.  This strategy fundamentally consists in the removal of proteins whose corresponding transcript is not expressed according to the sample-matched transcriptome. The impact of the filtering strategy can be then assessed by building a new graph of filtered peptide-to-protein mappings, assessing its proportion of multi-protein CCs and visually inspecting peptide-to-protein mappings for ambiguous protein identifications of interest.
...
* [Build a graph from shotgun proteomic identifications and efficiently calculate its connected components (CCs) to quantify ambiguity of protein identifications;](#partA)
...
================================================================================
  ...
  The required input are proteomic identifications obtained by shotgun proteomics and, more precisely, all valid peptide identifications and the proteins
they map on, together with peptide-to-protein mapping information. When the objective is to assess ambiguity of those protein identifications directly obtained from identified peptides, proteins should be all proteins peptides map on, prior to any protein inference, 
The required input must be in the form of a tab-delimited incidence matrix with peptides along the rows and proteins along the columns. Each cell must report 1 or 0 to indicate whether or not the peptide maps on the corresponding protein. Protein identifiers (column names) should be in the Ensembl format (i.e. ENSPXXX for human, ENSMUSPXXX for mouse) or, if protein contaminants, in any format followed by a unique tag to indicate them as contaminants. Peptide identifiers (row names) can be in any format (amino acid sequence, numeric identifiers, ...). Column and row names should be provided in separate files, with one identifier per line. 
...
Incidence matrices generated from proteomic datasets can be quite large but they can be easily read in input chunk by chunk using the readIncM() function.
This function requires three inputs:  
* the name of the tab-delimited file containing the incidence matrix cell values - with no column or row names - (peptide-to-protein mappings);  
* the name of the file containing protein identifiers (column names of the matrix), one per line;  
* the name of the file containing peptide identifiers (row names of the matrix), one per line.  

Read the incidence matrix describing peptides and corresponding identified in a proteomic experiment and peptide-to-protein mapping information
```{r}
...
In order to decrease the computational cost of calculating graph connected components, we reduce the size of the input incidence matrix by removing all proteins not sharing peptides and all peptides exclusively mapping to these proteins. Only ambiguous protein identifications are left, which is proteins connected by shared peptides and belonging to multi-protein CC.
```{r}
...
and calculate graph connected components, using the getCC() function. 
The function provides in output a list of two elements:  
i. a graph of protein connections (by shared peptides) as described by the adjacency matrix;   
ii. a list of vectors (one per connected component) enumerating protein members of each connected component  

```{r}
# Calculate graph and connected components
...
The proportion of single- or multi-protein CCs and the size of multi-protein CCs represent a measure for the level of ambiguity of protein identifications and they can be obtained using the CC.stats() function.
The above calculated CCs exclusively are multi-protein CCs since they were calculated on the reduced incidence matrix from step 2, which only contains proteins sharing at least one peptide. This is specified setting the *reducedIncM* parameter to TRUE when using the CC.stats() function. Single-protein CCs are obtained as all those proteins from the original incidence matrix which do not belong to multi-protein CCs.
...
It can be calculated using the peptideStats() function.
```{r}
...
Given an ambiguous protein identification of interest (a protein identification with shared peptides), identify the
multi-protein CC it belong to, all protein members of that CC and all peptides mapping on them. This information is then
represented by a bipartite graph of peptide-to-protein mappings.

First generate an objects containing all peptides and peptide-to-protein mapping information for each CC using the cc.peptides.incM() function.
The function generates in output a list of two elements:    
...
Then plot, for the protein of interest, a bipartite graph representing the CC it belongs to:
```{r, fig.height=7}
prot <- "ENSP261"
subgraphCC <- plotCCs(prot = prot
        , cc.proteins = cc.multProteins
        , cc.subincM = cc.peptides.incM$cc.subincM
        , incM = incM
        , tagProt = "ENSP"
        , tagContam = "Contam")
 plot.igraph(subgraphCC$g, layout = layout_as_bipartite,
       edge.width = 1,
...
===============================================================================
  ...
  The required input is the same as in section 2: a tab-delimited incidence matrix describing identified peptides (along the rows) and proteins they map on (along the columns). No prior protein inference should be performed, When the objective is to assess ambiguity of protein identifications directly obtained from identified peptides.  

Filtering can be performed in three different ways, indicated by the 'remove' parameter of the postHocFilter() function:  
i. remove all proteins whose corresponding transcript is not expressed according to the sample-matched transcriptome; then remove all specific or shared peptides exclusively mapping on them   (parameter remove="all");  
ii. remove proteins whose corresponding transcript is not expressed, if they are only identified by shared peptides; then remove peptides exclusively mapping on them (parameter remove="sharedOnly");  
iii. remove proteins whose corresponding transcript is not expressed, if they are only identified by shared peptides and if those peptides are shared with at least one protein not filtered out (*i.e.* a protein whose transcript is expressed); no peptide needs to be removed (parameter remove="sharedNoRemove").  

## Read in input the incidence matrix describing peptide-to-protein mappings

Read the incidence matrix describing peptides identified in a shotgun proteomic experiment and proteins they map on
```{r}
incM_filename <- system.file( "extdata"
                        , "incM_Example"
                        , package = "CCs4prot"
...
Perform transcriptome-informed post-hoc filtering using the postHocFilter() function, which requires three inputs:  
  1. the name of the file containing the incidence matrix of peptide-to-protein mappings  
  2. the name of the file containing the transcripts expressed in the sample-matched transcriptome)  
  3. the name of the file containing for each protein ID the corresponding transcript ID. [^1]  

[^1]: It can be easily obtained from the Ensembl FASTA protein sequence database files (*i.e.*, Homo_sapiens.GRCh38.pep.all.fa for the human genome assembly GRCh38), publicly available at https://www.ensembl.org/info/data/ftp/index.html. The following bash command generates a tab-delimited file containing proteinID and corresponding transcriptID:
...
# Check incidence matrix size after transcriptome-informed filtering
dim(incM_filt)
```
...
The proportion and size of multi-protein CCs is calculated and compared to that obtained from the original graph of peptide-to-protein mappings, prior to transcriptome-informed filtering.
Their comparison informs on the amount of ambiguity in protein identifications obtained after
transcriptome-informed filtering, compared to original proteomic identifications.
...
# component in the object cc.multProteins_filt
...
In the above example, transcriptome-informed filtering does not lead to an increase
in the percentage of single-protein CCs (i.e. the percentage of non ambiguous protein
identifications); however, it yields a reduction in the size of multi-protein CCs 
(i.e., the number of protein members), overall allowing to reduce ambiguity of 
protein identifications.

### Compare the proportion of shared peptides obtained before and after filtering
The proportion of shared peptides is calculated and compared to that obtained from the original graph of peptide-to-protein mappings, prior to transcriptome-informed filtering.
Their comparison informs on the amount of ambiguity in protein identifications obtained after
transcriptome-informed filtering, compared to original proteomic identifications.


Calculate the proportion of shared peptides versus specific peptides obtained after transcriptome-informed filtering
```{r}
peptideStatsOut_filt <- peptideStats(incM = incM_filt)
```

Compare the proportion of specific versus shared peptides obtained after transcriptome-informed filtering 
```{r}
...
ggplot(data = comp
    , aes(x = as.factor(Filter), y = as.numeric(as.vector(Perc_sharedPeptides)))) +
      geom_bar(stat = "identity") +
      theme_classic() +
      xlab("") +
...
In order to visually inspect the impact of transcriptome-informed filtering on one ambiguous protein identification of interest, for example protein ENSP00000477916, plot the bipartite graphs of peptide-to-protein mappings representing the CC to which it belongs.
Compare the CC graph before or after transcriptome-informed filtering.

```{r, fig.height=14}
# Extract peptides and peptide-to-protein mappings for each CC after filtering
cc.peptides.incM_filt <- CC.composition(cc.multProteins_filt
                                            , incM = incM_filt)

# Generate, for the protein of interest, a bipartite graph representing the CC 
# it belongs to, before and after transcriptome-informed filtering.
prot="ENSP261"
subgraphCC_beforeFilter <- plotCCs(prot = prot
                      , cc.proteins = cc.multProteins
                      , cc.subincM = cc.peptides.incM$cc.subincM
                      , incM = incM
                      , tagProt = "ENSP"
...
subgraphCC_afterFilter <- plotCCs(prot = prot
